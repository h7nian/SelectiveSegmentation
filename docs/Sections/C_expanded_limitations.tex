...
\(M\to\infty\), and the \(1/c\) dependence is weakest at very low coverage. Thus a
...
Even the predeclared five-native-binary-dataset design and two model families
...
image-cluster bootstrap quantifies held-out image variation for a fixed fitted
checkpoint; its percentile intervals and tail probabilities are approximate, and
their Holm-transformed values do not carry an exact finite-sample family-wise
error guarantee. With one predeclared training seed they also do not include
optimization or training-sample variability. Repeated training runs and external-site validation would be
needed for that broader claim. Moreover, model conditions reuse the same held-out
images within each dataset; condition-specific rows are paired evaluations, not
independent images.
