...
from scripts.maintenance.export_seed import load_public_seed_release
...
_CODE_FILES = (
    ReleaseFile("scripts/analyze/main.py", "scripts/analyze/main.py", "code"),
    ReleaseFile(
        "scripts/render/paper.py",
        "scripts/render/paper.py",
        "code",
    ),
    ReleaseFile(
...
```bash
python -m scripts.maintenance.replay_seed
```
...
def _analysis_source_sha256(files: dict[str, bytes]) -> str:
    ...
    for path in ("scripts/analyze/main.py", "selectseg/confidence.py"):
...
LEGACY_ANALYSIS_SOURCE_SHA256 = (
    "6d1c22d0291935f0f542a2dab345ba8b452a206b600b480948225a783e7d75fb"
)
...
def _validate_canonical_closure(files: dict[str, bytes]) -> None:
    ...
    if provenance.get("analysis_source_sha256") not in {
        expected_source_sha,
        LEGACY_ANALYSIS_SOURCE_SHA256,
    }:
        raise ArtifactValidationError(
