...
from scripts.analyze import seed as seed_analysis_analyzer
...
from scripts.submit.seed import (
...
from selectseg.studies.diagnostics import load_binary_diagnostics
    ...
    for campaign_binding in downstream_binding["campaigns"]:
        ...
        for artifact_index, artifact in enumerate(lock["artifacts"]):
            ...
            for simulation in simulations:
                argv.extend(("--input", str(simulation)))
            argv.extend(("--output-root", lock["paths"]["assembly_output_root"]))
            command = slurm_command(
                job_name=(
                    f"selseg-seed-s{seed}-selseg-assemble-{dataset}-{condition}"
                )[:128],
                partition=partition,
                cpus=1,
                memory="4g",
...
def _renderer_source_sha256():
    ...
    paths = (
        root / "scripts" / "render" / "seed.py",
        root / "scripts" / "analyze" / "seed.py",
