...
from selectseg.provenance import EVAL_DATASETS
...
def plan_jobs(output_dir=DEFAULT_OUTPUT_DIR):
    ...
    for dataset in EVAL_DATASETS:
        ...
        command = slurm_command(
            job_name=f"selseg-science-data-{dataset}",
            partition=partition_request,
            cpus=2,
            memory="8g",
            time_limit="06:00:00",
            argv=("python", "-m", "selectseg.provenance", "build-dataset",
                  "--dataset", dataset, "--data-root", "data", "--output",
                  output.as_posix()),
        )
        jobs.append(
            PlannedJob(
