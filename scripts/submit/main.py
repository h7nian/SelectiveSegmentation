...
GPU_ACCOUNT = "ssafo"
SLURM_RUNNER = "scripts/slurm/run.sbatch"
...
def slurm_command(
    *,
    ...
    """Build one explicit non-array Slurm command for the generic runner."""

    if not argv:
        raise ValueError("Slurm command argv must be nonempty")
    command = [
        "sbatch",
        "--parsable",
        "--job-name",
    ...
    if gres is not None:
        command.extend(("--gres", str(gres)))
    command.extend((SLURM_RUNNER, *map(str, argv)))
    if any(token == "--array" or token.startswith("--array=") for token in command):
...
def _scientific_plan_binding(config, *, mode="fast"):
    ...
    from selectseg.provenance import load_root_lock, verify_root_lock
...
def plan_freeze_jobs(config):
    ...
    for condition_index, condition in enumerate(config.data["conditions"]):
        ...
        if condition["checkpoint"]:
            argv.extend(("--checkpoint", condition["checkpoint"]))
        if condition.get("freeze_limit") is not None:
            argv.extend(["--limit", str(condition["freeze_limit"])])
        if scientific_plan is not None:
            from selectseg.provenance import condition_input_identity
        ...
        jobs.append(PlannedJob(phase="freeze", key=key, command=command))
    return tuple(jobs)
        ...
        if checkpoint is not None:
            ...
            if not expected_checkpoint.is_file():
                raise FileNotFoundError(
                    f"locked checkpoint does not exist: {expected_checkpoint}"
                )
            # Artifact manifests intentionally store portable provenance paths.
            # Their spelling need not match the campaign's executable path; the
            # immutable content hash is the scientific identity shared by both.
            if _sha256(expected_checkpoint) != checkpoint["sha256"].lower():
                raise ValueError(
...
def plan_assemble_jobs(config, campaign_lock, *, allow_existing_output=False):
    ...
    for artifact_index, artifact in enumerate(lock["artifacts"]):
        ...
        jobs.append(PlannedJob(phase="assemble", key=key, command=command))
    expected_count = len(config.data["conditions"])
    if len(jobs) != expected_count or len({job.key for job in jobs}) != expected_count:
