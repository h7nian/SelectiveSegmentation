...
from selectseg.binary_seed_downstream import (
...
def _profile_command_prefix(experiment, job_name):
...
def plan_training_jobs(binding):
    ...
    for experiment in iter_experiments(binding["spec"]):
        ...
        command = _profile_command_prefix(experiment, job_name) + (
...
def _validate_downstream_job_isolation(jobs, *, phase):
    ...
    wrappers = {
        "common": "scripts/slurm/score_binary_common.sbatch",
        "score": "scripts/slurm/score_binary_simulation.sbatch",
        "assemble": "scripts/slurm/assemble_binary_simulations.sbatch",
        "diagnose": "scripts/slurm/diagnose_binary_artifact.sbatch",
    }
    expected_flags = {
        "common": {
            "--artifact-manifest": 1,
    ...
    for job in jobs:
        ...
        if command.count(wrappers[phase]) != 1:
            raise RuntimeError(f"seed {phase} job has the wrong Slurm wrapper")
...
def plan_analysis_job(
    ...
    *,
    ...
    from scripts.analyze_binary_seed_extension import validate_analysis_inputs
...
def plan_render_job(
    ...
    *,
    ...
    from scripts.render_binary_seed_extension import load_analysis
