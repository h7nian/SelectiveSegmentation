...
from scripts.submit.m128 import CPU_PARTITIONS, plan_m128_jobs
...
def test_planner_is_one_cpu_job_per_condition_and_cycles_allowed_partitions(
    ...
    assert all(job.command[job.command.index("--time") + 1] == "12:00:00" for job in jobs)
    assert "#SBATCH --gres" not in wrapper
    assert "#SBATCH --gpus" not in wrapper
    assert all("selectseg.studies.m128" in job.command for job in jobs)
