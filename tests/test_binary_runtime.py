...
from scripts.render_binary_runtime import (
    OUTPUT_NAME,
    load_analysis,
    render_analysis,
    validate_analysis,
...
from scripts.submit_binary_runtime import CPU_PARTITIONS, plan_runtime_jobs
from scripts.submit_binary_runtime_ladder import plan_runtime_ladder_jobs
from scripts.submit_binary_simulations import write_campaign_lock
...
RUNTIME_LADDER_SPEC = (
    ROOT / "configs" / "auxiliary" / "binary_runtime_ladder-v2.json"
)
RUNTIME_LADDER_LOCK = (
...
def _write_artifact(root, dataset, condition, *, num_samples):
    ...
    for index in range(num_samples):
        ...
        probability = np.linspace(
            0.01, 0.99, height * width, dtype=np.float32
...
def _locked_campaign(tmp_path, *, num_samples=1):
    ...
    lock_path, lock_sha256 = write_campaign_lock(
        lock, tmp_path / "campaign.lock.json"
    )
...
def test_runtime_scorer_excludes_io_uses_fixed_grid_and_is_append_only(
    ...
    artifacts, lock_path, lock_sha256, _ = _locked_campaign(
        tmp_path, num_samples=1
    )
    ...
    assert "artifact manifest and payload I/O" in manifest["benchmark_scope"][
        "excluded"
    ]
    ...
    for method in METHODS:
        assert len(
            {row["result_sha256"] for row in records if row["method"] == method}
        ) == 1
...
def test_runtime_planner_is_exactly_one_cpu_job_per_locked_artifact(tmp_path):
    ...
    assert [job.command[job.command.index("--partition") + 1] for job in jobs] == [
        CPU_PARTITIONS[index % 3] for index in range(16)
    ]
    for job in jobs:
        command = job.command
        assert command[0] == "sbatch"
        assert "scripts/slurm/benchmark_binary_runtime.sbatch" in command
...
def test_runtime_ladder_lock_hash_changes_runtime_identity():
    ...
    assert _runtime_id(
        **identity, benchmark_lock_sha256="a" * 64
    ) != _runtime_id(**identity, benchmark_lock_sha256="b" * 64)
...
def test_runtime_ladder_planner_is_locked_and_one_job_per_condition(tmp_path):
    ...
    for job in jobs:
        ...
        assert "scripts/slurm/benchmark_binary_runtime.sbatch" in job.command
...
def _write_synthetic_runtime_inputs(
    ...
    *,
    ...
    for condition_index, ((dataset, condition), artifact_path) in enumerate(
        ...
        records = []
        method_digests = {
            method: hashlib.sha256(f"{dataset}/{condition}/{method}".encode()).hexdigest()
            for method in methods
        }
        for repetition in range(4):
...
def test_strict_runtime_analysis_and_compact_renderer(tmp_path):
    ...
    corrupt["conditions"][0]["methods"]["m32_joint"][
