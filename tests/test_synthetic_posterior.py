...
from scripts.analyze_synthetic_posterior import analyze
from scripts.render_synthetic_posterior import main as render_main
from scripts import submit_synthetic_posterior as submit_synthetic
...
def test_repository_lock_and_job_counts_are_exact():
    ...
    for job in pilot:
        ...
        assert command.count("scripts/slurm/run_synthetic_posterior.sbatch") == 1
    ...
    for coupling_index, coupling in enumerate(spec["grid"]["couplings"]):
        for sharpness_index, sharpness in enumerate(spec["grid"]["sharpness_levels"]):
            ...
            manifest = {
                ...
                "artifact_type": "selectseg.synthetic_posterior_cell",
